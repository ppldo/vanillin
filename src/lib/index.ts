...
function reduceGlobalStyles(rules: IGlobalRule[]): GlobalStyle[] {
  return ld.chain(rules).groupBy((r) => {return r.vanillaSelector.serialize()}).values().map((globalRules: IGlobalRule[]) => {
...
function reduceRegularStyles(rules: IRegularRule[]): RegularStyle[] {
  return ld.chain(rules).groupBy((r) => {return r.varName}).values().map((regularRulesByVar: IRegularRule[]) => {
    let allDeps = new Set<string>()
    ...
    const selectorConfs: ISelectorConf[] = ld.chain(regularRulesByVar)
      .groupBy(r => {return r.vanillaSelector.serialize()})
      .values()
